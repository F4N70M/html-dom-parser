...
### Пример 2: Работа со ссылкой и сущностями {#example-entities}
...
$entities = $element->getEntities(); // EntityListInterface
foreach ($entities as $entity) {
    echo 'Тип: ' . $entity->getType() . PHP_EOL;
    echo 'Начало: ' . $entity->getStart() . PHP_EOL;
    echo 'Конец: ' . $entity->getEnd() . PHP_EOL;
    echo 'Атрибуты: ' . print_r($entity->getAttributes(), true) . PHP_EOL;
}
// Или для компактного вывода:
// print_r($entities->toArray());
```

Подробнее о сущностях форматирования читайте в разделе [Модель данных элемента](./02-core--01-data-model.md#entities).
...
- [Модель данных элемента](./02-core--01-data-model.md) — подробнее о Data, Label и Entities
- [Система контекстов](./02-core--02-context-system.md) — как это работает внутри
- [Утилиты](./02-core--03-utilities.md) — DataResolver и InlineCollapser
...
Пожалуйста, предоставьте исходные файлы для написания следующего документа:
`02-core--01-data-model.md`1. `01-general-information--03-quick-start.md`
```markdown
[← К оглавлению](../README.md#-документация)
