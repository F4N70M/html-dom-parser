...
Библиотека использует класс `ErrorConstant` для определения уровней серьезности ошибок. Полный список констант доступен в [Справочнике API](./04-appendix--02-api-reference.md#errorconstant).
...
Вы можете настроить, при каких уровнях ошибок выбрасывать исключение [`HtmlDomParserException`](./04-appendix--02-api-reference.md#htmldomparserexception):
...
if ($errorHandler->hasErrors()) {
    ...
    foreach ($errorHandler->getErrors() as $error) {
        /** @var ErrorElementInterface $error */
...
- [Справочник API: ErrorConstant](./04-appendix--02-api-reference.md#errorconstant)
